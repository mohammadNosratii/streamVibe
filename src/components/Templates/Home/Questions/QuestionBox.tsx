        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 mt-16 w-[95%] mx-auto lg:gap-28'>
            <Accordion>
                {data.slice(0, 4).map((question) => (
                    <AccordionItem key={question.id} aria-label={question.title} title={question.title} indicator={({ isOpen }) => (isOpen ? <MinusIcon /> : <PlusIcon className='dark:fill-current' />)} startContent={
