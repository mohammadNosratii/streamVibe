...
export default function VerifyEmailStatus() {
    ...
    const { email, key } = search
    const { isSuccess, mutate: verifyEmailMutate, isPending: verifyEmailPending } = useVerifyEmailApi()
    const { mutate: resendEmailMutate, isPending: resendEmailPending } = useResendEmailApi()
    ...
    useEffect(() => {
        if (key) {
            verifyEmailMutate({ key })
        }
    }, [key, verifyEmailMutate])
    return (
        <>
            {!key ? <div className="space-y-8">
            ...
            </div> : verifyEmailPending ?
                <div className="flex items-center gap-2 justify-center">
                    <p className="text-2xl font-manropeSemiBold dark:text-white">Verifying</p>
                ...
                isSuccess ?
                    ...
                    <div className="space-y-4">
                        ...
                        <p className="font-manropeMedium text-gray-60">Your Email has not been verified. Please Try again later</p>
                        <Button className="dark:bg-black-10 w-full" onClick={() => navigate({ to: "/register" })}>Back To Registeration</Button>
                    </div>
            }
        </>
    )
